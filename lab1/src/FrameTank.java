...
public class FrameTank {
    ...
    public FrameTank() {
        ...
        btnCreate.addActionListener(e -> setTank());
        ...
        list = new JComboBox<>(new String[]{"1 пушка", "2 пушки", "3 пушки"});
    ...
    private void setDirection(JButton button) {
        ...
        switch (name) {
            case "up":
                drawPanel.getTank().moveLocomotive(Direction.Up);
                break;
            case "down":
                drawPanel.getTank().moveLocomotive(Direction.Down);
                break;
            case "left":
                drawPanel.getTank().moveLocomotive(Direction.Left);
                break;
            case "right":
                drawPanel.getTank().moveLocomotive(Direction.Right);
                break;
        }
        frame.repaint();
    ...
    private void setTank() {
        drawPanel.setTank(new Tank(150, 1500, new Color(34, 139, 34), Color.RED, true, true, list.getSelectedIndex() + 1));
        drawPanel.getTank().setPosition((int) (Math.random() * 100 + 10), (int) (Math.random() * 100 + 10), 850, 450);
        frame.repaint();
    }
}
